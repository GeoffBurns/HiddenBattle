...
/* global global, require, describe, it, expect, beforeEach, jest */
...
jest.mock('../selection/dragndrop.js', () => ({
  dragNDrop: {
...
describe('ScoreUI', () => {
  ...
  describe('display', () => {
    ...
    // not working
    /*
    it('should display sunk count and total', () => {
      const mockShips = [
        { sunk: true },
        { sunk: true },
        { sunk: false },
      ...
      scoreUI.display(mockShips, 0)
      expect(scoreUI.sunk.textContent).toBe('2 / 4')
    })
*/
    ...
    /*
    it('should handle no sunk ships', () => {
      const mockShips = [{ sunk: false }, { sunk: false }]
      scoreUI.display(mockShips, 10)
      expect(scoreUI.sunk.textContent).toBe('0 / 2')
    })

    it('should handle all ships sunk', () => {
      const mockShips = [{ sunk: true }, { sunk: true }]
      scoreUI.display(mockShips, 15)
      expect(scoreUI.sunk.textContent).toBe('2 / 2')
    })
  })
*/
    describe('displacementDescription', () => {
      ...
      /*
    it('should return "medium" for mid ratio', () => {
      expect(scoreUI.displacementDescription(0.53)).toBe('medium')
    })

    it('should return "tight" for high ratio', () => {
      expect(scoreUI.displacementDescription(0.68)).toBe('tight')
    })
*/
    ...
    it('should handle boundary values', () => {
      expect(scoreUI.displacementDescription(0.02)).toBe('lonely')
      expect(scoreUI.displacementDescription(0.15)).toBe('very scattered')
      expect(scoreUI.displacementDescription(0.81)).toBe('compact')
    })
...
*/
  ...
  describe('buildShipTally', () => {
    ...
    it('should call altBuildTally with ships and no weapons', () => {
      const spy = jest.spyOn(scoreUI, 'altBuildTally')
      const mockShips = []
      scoreUI.buildShipTally(mockShips)
      expect(spy).toHaveBeenCalledWith(mockShips, [], undefined)
    })
  })

  describe('buildTallyFromModel', () => {
    it('should call altBuildTally with model data', () => {
      global.document.createElement = jest.fn(() => ({
        className: '',
        textContent: '',
        style: {},
      ...
      const mockModel = {
        ships: [],
        loadOut: { weaponSystems: [] }
      }
      const mockViewmodel = {}
      scoreUI.buildTallyFromModel(mockModel, mockViewmodel)
      expect(spy).toHaveBeenCalled()
    })
  })

  describe('buildTally', () => {
    it('should call altBuildTally with weapons', () => {
      global.document.createElement = jest.fn(() => ({
        className: '',
        textContent: '',
        style: {},
  ...
  describe('addShipTally', () => {
    it('should call altBuildTally with false for viewModel', () => {
      global.document.createElement = jest.fn(() => ({
        className: '',
        textContent: '',
        style: {},
      ...
      const spy = jest.spyOn(scoreUI, 'altBuildTally')
      scoreUI.addShipTally([])
      expect(spy).toHaveBeenCalledWith([], [], null, false)
    })
  })

  describe('altBuildTally', () => {
    beforeEach(() => {
      global.document.createElement = jest.fn(() => ({
        className: '',
        textContent: '',
    ...
    it('should reset tally box', () => {
      const spy = jest.spyOn(scoreUI, 'resetTallyBox')
      scoreUI.altBuildTally([], [], null)
      expect(spy).toHaveBeenCalled()
    })

    it('should handle empty ship list', () => {
      expect(() => {
        scoreUI.altBuildTally([], [], null)
      }).not.toThrow()
    })

    it('should toggle tally title visibility', () => {
      scoreUI.altBuildTally([], [], null)
      // If tally-title exists, it should be toggled
     */
    })
  })
})
