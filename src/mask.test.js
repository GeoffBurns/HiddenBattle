...
describe('Mask - additional methods and edge cases', () => {
  ...
  describe('applyMap and orbit', () => {
    it('should apply identity map and return same bits', () => {
      mask.set(0, 0)
      const idMap = mask.transformMaps.id
      expect(mask.applyMap(idMap)).toBe(mask.bits)
    })

    it('should return 8 symmetries in orbit', () => {
      mask.set(0, 0)
      const orbit = mask.orbit(mask.transformMaps)
  ...
  describe('classifySymmetry', () => {
    it('should classify empty mask as C1', () => {
      const ss = mask.symmetries
      expect(ss.length).toBe(1)
      const s = ss[0]
      const n = mask.normalized()
      expect(s).toBe(n)
      const k = mask.order
      expect(k).toBe(1)
