...
describe('DraggedShip', () => {
  ...
  beforeEach(() => {
    ...
    mockShip = {
      ...
      placePlaceable: jest.fn().mockReturnValue([{ cell: 'data' }]),
      addToGrid: jest.fn(),
      cells: [{ r: 0, c: 0 }]
    }
  ...
  describe('addCurrentToShipCells', () => {
    it('should add placeable to ship cells', () => {
      ...
      expect(mockShip.placePlaceable).toHaveBeenCalledWith(mockPlaceable, 2, 3)
    })
  ...
  describe('placeCells', () => {
    it('should add cells to ship when placement is valid', () => {
      ...
      expect(mockShip.placePlaceable).toHaveBeenCalled()
      expect(result).toEqual(mockShip.cells)
    })
    ...
    it('should return null when placement is invalid', () => {
      ...
      const result = draggedShip.placeCells(2, 3, shipCellGrid)
      expect(mockShip.placePlaceable).not.toHaveBeenCalled()
    ...
    it('should account for cursor offset', () => {
      ...
      expect(mockShip.placePlaceable).toHaveBeenCalledWith(
        mockPlaceable,
        4, // 5-1
        6 // 8-2
      )
  ...
  describe('place', () => {
    it('should push ship to placedShipsInstance when valid', () => {
      const { placedShipsInstance } = require('./PlacedShips.js')
      const shipCellGrid = { grid: 'data' }
      draggedShip.cursor = [0, 0]
      mockPlaceable.canPlace.mockReturnValue(true)
    ...
    it('should return result from placedShipsInstance.push', () => {
      const { placedShipsInstance } = require('./PlacedShips.js')
      const shipCellGrid = { grid: 'data' }
      draggedShip.cursor = [0, 0]
      mockPlaceable.canPlace.mockReturnValue(true)
